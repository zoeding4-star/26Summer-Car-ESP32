...
/*
 * 循迹用的软件 tjpgd 不能占用 ROM / esp_jpeg 的 jd_prepare 符号。
 * ESP32-S3 上 CONFIG_JD_USE_ROM=y 时，esp_jpeg 按 ROM 的 JDEC 布局编译；
 * 若链接到本文件的 jd_prepare，第一次彩色解码会在 jpeg_decode_in_cb
 * 里 Cache panic（Guru Meditation: Cache disabled but cached memory region accessed）。
 */
#define jd_prepare  cam_jd_prepare
#define jd_decomp   cam_jd_decomp
