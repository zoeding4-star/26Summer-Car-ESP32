/**
 * 全程：摄像头循迹 + 超声波避障 + 屏幕，停 3 秒后再推球入网。
 *
 * 循迹/避障对齐 cam_line_avoid_ui.c；推球对齐 catch.c。
 * 终止 T（横着全黑后再全白）停车 → 静止 3 秒 → 找球推球（红蓝各一次）。
 *
 * 引脚：USB 摄像头占用 GPIO19/20；右前霍尔接到 GPIO4/5。
 */
...
#define TIMEOUT_US      8000    /* ~1.3m，避免每次测距卡死看门狗 */
...
static const char *lcd_status_text(void);
...
static int64_t g_push_until = 0;
static int64_t g_pulse_ready_at = 0;
static int64_t g_dir_t0 = 0;
static bool g_search_left = true;
static bool g_search_centering = false;
static bool g_search_reversed = false;
static bool g_orbit_left = false;
static bool g_orbit_reversed = false;
static float g_orbit_radius_m = 0.10f;
static int64_t g_orbit_pause_until = 0;
    ...
    while (1) {
        ...
        if (xTaskGetTickCount() - last_text >= pdMS_TO_TICKS(200)) {
            ...
            if (lbl_wheel_a && lbl_wheel_b && lbl_wheel_d && lbl_dist && lbl_phase) {
                ...
                snprintf(buf, sizeof(buf), "PH: %s", lcd_status_text());
...
static bool decode_mjpeg_rgb_scale(const uint8_t *jpg, int len, esp_jpeg_image_scale_t scale);
...
{
    if (!s_gray || !decode_mjpeg_rgb_scale(jpg, len, JPEG_IMAGE_SCALE_1_8)) {
        return false;
    }
    int n = s_img_w * s_img_h;
    for (int i = 0; i < n; i++) {
        const uint8_t *p = s_rgb + (size_t)i * 3;
        s_gray[i] = (uint8_t)((p[0] * 30 + p[1] * 59 + p[2] * 11) / 100);
    }
    return true;
}
...
static const char DBG_HTML[] =
    ...
    "<li>直角/锐角：只记方向，丢线后再原地转。</li><li>终止停车后静止 3 秒，再找红/蓝球推入荧光绿网，各推一次。</li>"
    "</ol>"
    "<script>"
    ...
    "'推球  状态：'+inf.st+'    目标：'+inf.ball+'    已推 '+(inf.done?inf.done.n:0)+'/2'+"
    "'\\n球距车头 '+inf.dcm+'cm  球-网 '+inf.ndcm+'cm  对齐角 '+inf.ang+'°'+"
    "'\\nvy='+inf.vy+' om='+inf.om+'  解码 '+inf.ms+'ms';"
    "}else{"
    "const r0=(inf.roi0|0),r1=(inf.roi1||w);"
    "const yTop=(inf.scan_y&&inf.scan_y.length)?(inf.scan_y[inf.scan_y.length-1]||0):0;"
    "const yBot=(inf.scan_y&&inf.scan_y.length)?(inf.scan_y[0]||h):h;"
    ...
    "c2.strokeStyle='#cc0';"
    "if(inf.scan_y){for(const y of inf.scan_y){c2.beginPath();c2.moveTo(r0,y+0.5);c2.lineTo(r1,y+0.5);c2.stroke();}}"
    ...
    "if(inf.blobs){for(const b of inf.blobs){c2.strokeRect(b.x-b.w/2,b.y-1,Math.max(2,b.w),3);}}"
...
static void begin_catch_wait(const char *why)
{
    stop_motors();
...
static void follow_tick(const Sight *p)
{
    if (g_has_avoided && (p->t_bar || p->type == VIEW_BAR)) {
        if (!g_t_seen) {
            g_t_seen_t0 = esp_timer_get_time();
        }
        g_t_seen = true;
        g_t_white = 0;
        drive(BASE_SPEED, 0.0f);
    ...
    if (g_t_seen) {
        ...
        if (g_t_seen_t0 > 0 &&
            (esp_timer_get_time() - g_t_seen_t0) > (int64_t)T_DRIVE_MS * 1000) {
            begin_catch_wait("终止线：压过 T 后停车");
            return;
        }
        g_t_white = 0;
        drive(BASE_SPEED, 0.0f);
...
static void control_once(void)
{
    ...
    case PHASE_FWD:
        ...
        if ((now - g_phase_t0) > (int64_t)AVOID_FWD_MS * 1000) {
            ESP_LOGI(TAG, "右移找线 vx=-%.0f", STRAFE_BACK_SPEED);
    ...
    case PHASE_STRAFE_BACK:
        ...
        if (p.has_black || p.near_ok) {
            ...
            begin_catch_wait("回线超时");
        }
        break;

    case PHASE_STOP:
        begin_catch_wait("循迹结束");
        break;
    }
...
/* ==================== 推球（catch.c，常量/电机与循迹分开） ==================== */
#define CATCH_APPROACH_SPEED    40.0f
#define CATCH_PUSH_SPEED        64.0f   /* 原 32 的 2 倍，快射 */
#define CATCH_BACKUP_SPEED      36.0f
#define CATCH_OMEGA_MICRO       12.0f
#define CATCH_OMEGA_MACRO       18.0f
#define CATCH_ROT_MAX           22.0f
#define CATCH_SPIN_PWM          30.0f   /* 原地搜球 */
#define CATCH_SPIN_CENTER_PWM   26.0f   /* 对中，比搜球稍快 */
#define CATCH_SPIN_MS_PER_DEG   21      /* 随搜球 PWM 估 120° */
#define SEARCH_LEFT_MAX_MS      (120 * CATCH_SPIN_MS_PER_DEG)
#define CATCH_ORBIT_OMEGA       50.0f   /* 绕球力加大 */
#define CATCH_ORBIT_Y_M         0.10f
#define ORBIT_MS_PER_DEG        13      /* omega=50 时约 24°/300ms */
#define ORBIT_FIRST_DEG         60
#define ORBIT_REVERSE_DEG       150
#define ORBIT_FIRST_MS          (ORBIT_FIRST_DEG * ORBIT_MS_PER_DEG)
#define ORBIT_REVERSE_MS        (ORBIT_REVERSE_DEG * ORBIT_MS_PER_DEG)
#define ORBIT_TOTAL_MS          (ORBIT_FIRST_MS + ORBIT_REVERSE_MS)
#define CATCH_ORBIT_SCALE_D     0.5f
#define CATCH_ORBIT_SCALE_A     0.5f
#define CATCH_ORBIT_SCALE_B     2.0f
#define CATCH_ORBIT_B_BOOST     1.00f
#define CATCH_ORBIT_VX          66.0f
#define PULSE_ON_MS             120
#define PULSE_SPIN_ON_MS        20
#define PULSE_ORBIT_ON_MS       50
#define PULSE_BACKUP_ON_MS      220
#define PULSE_OFF_MS            80
#define PULSE_BRAKE_SCALE       0.32f
#define PULSE_BRAKE_MAX_MS      24
#define PUSH_MS                 500     /* 直行推球时长 */
#define BACKUP_MS               PUSH_MS /* 后退与这次直行同样长 */
#define APPROACH_STOP_CM        10.0f
...
#define CENTER_DEAD_PX          12      /* 允许略偏中，不必贴死画面中心 */
#define CENTER_MACRO_PX         18
#define ALIGN_VERT_PX           6
#define ALIGN_OK_DEG            10.0f
...
#define BALL_LOCK_RADIUS        14
#define BLACK_LINE_MIN_PCT      18
#define BALL_FAR_CROP_NUM       1
#define BALL_FAR_CROP_DEN       8
#define NEAR_Y_PCT              70
...
static void catch_spin(bool left)
{
...
static void catch_spin_hold(bool left, float pwm)
{
    float s = left ? -pwm : pwm;
    s = clampf(s, -(float)PWM_CAP, (float)PWM_CAP);
    MotorSpeed m = { s, -s, s };
    g_last_vy = 0.0f;
    g_last_om = s;
...
/*
 * 以球为圆心：v = ω × r。x 右、y 前，单位米；ω 与 vx 共用现有 IK 数值。
 * left：omega>0。
 */
static MotorSpeed catch_orbit_cmd(bool left)
{
    float y_m = g_orbit_radius_m;
    if (y_m < 0.05f) {
        y_m = CATCH_ORBIT_Y_M;
    }
    if (y_m > 0.20f) {
    ...
    float omega = left ? CATCH_ORBIT_OMEGA : -CATCH_ORBIT_OMEGA;
    float vx = omega * (y_m / L);
    float vy = -omega * (x_m / L);
    MotorSpeed m;
    m.D = -SIN_60 * vx + COS_60 * vy + L * omega;
    m.A =  SIN_60 * vx + COS_60 * vy - L * omega;
    m.B = vx + L * omega;
    float peak = fabsf(m.D);
    if (fabsf(m.A) > peak) {
        peak = fabsf(m.A);
    }
    if (fabsf(m.B) > peak) {
    ...
    if (peak > (float)PWM_CAP && peak > 0.001f) {
        float k = (float)PWM_CAP / peak;
        m.D *= k;
        m.A *= k;
        m.B *= k;
    }
...
static void pulse_orbit_around_front(bool left)
{
    ...
    g_last_vy = 0.0f;
    g_last_om = left ? CATCH_ORBIT_OMEGA : -CATCH_ORBIT_OMEGA;
    g_last_wheels = m;
    set_all_motors(&m);
}
...
static void enter_state(PushState st)
{
    ...
    g_dir_t0 = g_stage_t0;
    if (st == ST_SEARCH_BALL) {
        g_search_centering = false;
        g_search_reversed = false;
        if (!g_done_red) {
    ...
    if (st == ST_ALIGN) {
        /* 红球先顺时针，蓝球先逆时针 */
        g_orbit_left = (g_ball_kind != BALL_RED);
        g_orbit_reversed = false;
        g_orbit_pause_until = 0;
    }
    if (st == ST_PUSH) {
        g_push_until = esp_timer_get_time() + (int64_t)PUSH_MS * 1000;
    }
    if (st == ST_BACKUP) {
...
{
    return decode_mjpeg_rgb_scale(jpg, len, JPEG_IMAGE_SCALE_1_4);
}
...
static void pulse_center_on_x(int tx, float speed)
{
    ...
    /* 贴球不再前进时：用 orbit_test 横移点动绕球对中 */
    if (fabsf(speed) <= 1.0f) {
        if (abs(err) <= dead) {
            stop_motors();
            return;
        }
        pulse_orbit_around_front(err < 0);
        return;
    }
    if (abs(err) <= dead) {
...
static void catch_debug_update(const FrameSight *p)
{
    ...
    car_center(&ccx, &ccy);
    int n = snprintf(s_dbg_json, sizeof(s_dbg_json),
                     "{\"mission\":\"CATCH\",\"st\":\"%s\",\"ball\":\"%s\",\"ms\":%d,\"vy\":%.0f,\"om\":%.0f,"
...
static void on_push_success(void)
{
    ...
    stop_motors();
    if (balls_done() >= 2) {
        ESP_LOGI(TAG, "蓝球已推，停车结束");
        enter_state(ST_DONE);
        return;
    }
    ESP_LOGI(TAG, "推球结束，后退 %dms", PUSH_MS);
...
static void catch_control_once(void)
{
    ...
    case ST_SEARCH_BALL: {
        /* 红：只左转；蓝：只右转。慢转直到球大致在画面中央，然后直行推球。 */
        if (p->ball.found) {
            int err = p->ball.cx - s_img_w / 2;
            if (abs(err) <= center_dead_px()) {
                stop_motors();
                g_stop_hold++;
                if (g_stop_hold >= STOP_HOLD_FRAMES) {
                    ESP_LOGI(TAG, "%s 已对中 cx=%d，直行推球 %dms",
                             ball_name(g_ball_kind), p->ball.cx, PUSH_MS);
                    enter_state(ST_PUSH);
                    catch_drive(CATCH_PUSH_SPEED, 0.0f);
                }
                break;
            }
            g_stop_hold = 0;
        ...
        catch_spin_hold(g_search_left, CATCH_SPIN_CENTER_PWM);
        break;
    }
    ...
    case ST_PUSH: {
        /* 快射：全速直行 PUSH_MS 后停下，不再慢速带球、不等视觉入网 */
        if (esp_timer_get_time() >= g_push_until) {
            ESP_LOGI(TAG, "快射 %dms 结束", PUSH_MS);
            stop_motors();
            on_push_success();
            break;
        }
...
static const char *lcd_status_text(void)
{
    ...
    case MISSION_CATCH:
        switch (g_state) {
        case ST_SEARCH_BALL:    return "P:SEARCH";
        case ST_PUSH:           return "P:PUSH";
        case ST_BACKUP:         return "P:BACK";
        default:                return "P:CATCH";
        }
    case MISSION_DONE:
        return "P:DONE";
    default:
        return phase_name_ui(g_phase);
    }
}
...
static void line_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "全程任务：先循迹避障，停 3 秒后推球");
    while (1) {
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(line_task, "cam_final", 32768, NULL, 6, NULL, 0);
}
